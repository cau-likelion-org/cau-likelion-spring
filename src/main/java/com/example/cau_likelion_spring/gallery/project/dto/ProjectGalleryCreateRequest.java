...
/** thumbnailUrl/imageUrls는 POST /api/files/PROJECT로 미리 업로드해 받은 URL을 담는다. */
public record ProjectGalleryCreateRequest(
        ...
        @Schema(description = "대표 이미지 URL (POST /api/files/PROJECT로 미리 업로드한 URL, 선택)")
        String thumbnailUrl,

        @Schema(description = "이미지 URL 목록 (POST /api/files/PROJECT로 미리 업로드한 URL, 최소 1장, 최대 10장)")
