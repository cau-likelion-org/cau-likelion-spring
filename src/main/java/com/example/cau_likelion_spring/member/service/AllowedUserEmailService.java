...
public class AllowedUserEmailService {
    ...
    public List<AllowedUserEmailResponse> sync(Long generationId, AllowedUserEmailSyncRequest request) {
        ...
        List<String> duplicateEmails = request.items().stream()
                .map(AllowedUserEmailSyncRequest.AllowedUserEmailItem::email)
        ...
        if (!duplicateEmails.isEmpty()) {
            throw new CustomException(ErrorCode.DUPLICATE_EMAIL, "동일한 이메일이 목록에 중복으로 들어있습니다. email=" + duplicateEmails);
        }
        ...
        // 요청 목록에 없는(=화면에서 삭제된) 기존 항목 제거 - 한 번의 DELETE 문으로 처리
        allowedUserEmailRepository.deleteAllInBatch(remaining.values());
